...
class ToolSchemaTest {
    ...
    /**
     * Verifies that all tool classes in the tools package have unique NAME values
     * and that the count matches the number of tools registered in JmcMcpServer.
     */
    @Test
    void toolNamesAreUniqueAndMatchServerRegistration() throws Exception {
        // 1. Discover all *Tool classes in the tools package via classpath scanning
    ...
    /**
     * Discovers all *Tool classes in the io.github.deplague.jmcmcp.tools package
     * by scanning the classpath for .class files.
     */
    private Set<String> discoverToolClasses() throws Exception {
        String packageName = "io.github.deplague.jmcmcp.tools";
        String packagePath = packageName.replace('.', '/');

        Set<String> toolClasses = new TreeSet<>();

        // Scan all classpath entries for classes in the tools package
                    ...
                    for (java.io.File file : dir.listFiles()) {
                        ...
                        if (fileName.endsWith(".class") && !fileName.contains("$")) {
                            ...
                            try {
                                ...
                                // Not a tool class (e.g., SchemaUtil)
                            }
                        }
                    }
                }
    ...
    /**
     * Counts the number of `new *Tool(` instantiations in JmcMcpServer.java
     * to verify all tools are registered.
     */
    private int countRegisteredTools() throws Exception {
        // Read JmcMcpServer source from the filesystem (source files are not on the test classpath)
        String serverPath = System.getProperty("user.dir") + "/src/main/java/io/github/deplague/jmcmcp/JmcMcpServer.java";
        int count = 0;
        Pattern toolPattern = Pattern.compile("new\\s+(\\w+Tool)\\s*\\(");
