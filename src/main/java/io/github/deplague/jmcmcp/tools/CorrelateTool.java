...
public final class CorrelateTool {
    ...
    public String analyze(
        ...
        boolean showLockIo =
            "all".equals(dimension) || "lock_io_db".equals(dimension);
        boolean showCpuGc =
            "all".equals(dimension) || "cpu_gc".equals(dimension);
        ...
        sb.append(
            "`thread_contention` for lock details, or `io_hotspots` for I/O analysis.</agent_hint>\n"
        );
    ...
    private List<LockSite> extractLockSites(IItemCollection events, int topN) {
        ...
        for (String typeId : List.of(
            ...
            for (IItemIterable iterable : typeEvents) {
                IMemberAccessor<Object, IItem> monitorAccessor =
                ...
                if (
                    monitorAccessor == null || durationAccessor == null
                ) continue;
                ...
                for (IItem item : iterable) {
                    ...
                    LockSite site = sites.computeIfAbsent(key, k ->
                        new LockSite(monitorClass, topFrameCopy)
                    );
                    site.totalDurationMs += duration.clampedLongValueIn(
        ...
        result.sort(
            Comparator.comparingLong(LockSite::getTotalDurationMs).reversed()
        );
    ...
    private List<IoSite> extractIoSites(IItemCollection events, int topN) {
        ...
        for (String typeId : List.of(
            ...
            for (IItemIterable iterable : typeEvents) {
                ...
                String bytesAttr = typeId.contains("Read")
                    ? "bytesRead"
                    : "bytesWritten";
                ...
                IMemberAccessor<IQuantity, IItem> portAccessor =
                    portAttr != null
                        ? JfrItemUtils.getAccessor(iterable.getType(), portAttr)
                        : null;
                ...
                for (IItem item : iterable) {
                    ...
                    String topFrameCopy = topFrame;
                    IoSite site = sites.computeIfAbsent(key, k ->
                        new IoSite(typeId, endpointCopy, topFrameCopy)
                    );
                    site.totalDurationMs += duration.clampedLongValueIn(
                        UnitLookup.MILLISECOND
                    );
        ...
        result.sort(
            Comparator.comparingLong(IoSite::getTotalDurationMs).reversed()
        );
        return result.subList(0, Math.min(topN, result.size()));
    }

    private List<HotMethod> extractHotMethods(
        ...
        for (IItemIterable iterable : samples) {
            ...
            for (IItem item : iterable) {
                ...
                if (
                    topFrame.isEmpty() ||
                    topFrame.startsWith("...") ||
                    topFrame.startsWith("No stack")
                ) continue;
                methods.merge(topFrame, new HotMethod(topFrame, 1L), (a, b) ->
                    new HotMethod(a.methodName, a.sampleCount + b.sampleCount)
                );
            }
        }
        ...
        result.sort(
            Comparator.comparingLong(HotMethod::sampleCount).reversed()
        );
        return result.subList(0, Math.min(topN, result.size()));
    }
    ...
    private void appendLockIoCorrelation(
        ...
        long totalLockDuration = lockSites
            .stream()
            .mapToLong(LockSite::getTotalDurationMs)
            .sum();
        long totalIoDuration = ioSites
            .stream()
            .mapToLong(IoSite::getTotalDurationMs)
            .sum();

        int count = 0;
        for (LockSite lock : lockSites) {
            if (count >= topN) break;
            double ioUnderLockPct =
                totalIoDuration > 0
                    ? ((totalIoDuration * 100.0) /
                          (totalLockDuration + totalIoDuration))
                    : 0;
            String ioEndpoints = ioSites
                .stream()
                .limit(3)
                .map(s -> s.endpoint)
                .reduce((a, b) -> a + ", " + b)
        ...
        if (
            totalLockDuration > 0 && totalIoDuration > totalLockDuration * 0.5
        ) {
            sb.append(
    ...
    private void appendHotMethodLockCorrelation(
        ...
        if (hotMethods.isEmpty() || lockSites.isEmpty()) {
            sb.append(
                "Insufficient hot method or lock data for correlation.\n\n"
            );
            return;
        }
        ...
        long totalContentionMs = lockSites
            .stream()
            .mapToLong(LockSite::getTotalDurationMs)
            .sum();
        ...
        for (HotMethod method : hotMethods) {
            if (count >= topN) break;
            List<LockSite> relatedLocks = lockSites
                .stream()
                .filter(
                    ls ->
                        ls.topFrame.contains(method.methodName) ||
                        method.methodName.contains(ls.topFrame)
                )
                .toList();

            if (relatedLocks.isEmpty()) continue;

            long relatedDuration = relatedLocks
                .stream()
                .mapToLong(LockSite::getTotalDurationMs)
                .sum();
            double pct =
    ...
    private void appendHotMethodIoCorrelation(
        ...
        if (hotMethods.isEmpty() || ioSites.isEmpty()) {
            sb.append(
                "Insufficient hot method or I/O data for correlation.\n\n"
            );
            return;
        }
        ...
        for (HotMethod method : hotMethods) {
            if (count >= topN) break;
            List<IoSite> relatedIo = ioSites
                .stream()
                .filter(
                    io ->
                        io.topFrame.contains(method.methodName) ||
                        method.methodName.contains(io.topFrame)
                )
                .toList();

            if (relatedIo.isEmpty()) continue;

            long relatedDuration = relatedIo
                .stream()
                .mapToLong(IoSite::getTotalDurationMs)
                .sum();
            double pct =
                totalIoMs > 0 ? ((relatedDuration * 100.0) / totalIoMs) : 0;
    ...
    private void appendBottleneckChain(
        ...
        for (HotMethod method : hotMethods.stream().limit(2).toList()) {
            ...
            for (LockSite lock : relatedLocks) {
                ...
                sb.append(step++)
                    ...
                    .append(" [")
                    .append(
                        SchemaUtil.formatDuration(
                            lock.totalDurationMs / Math.max(lock.count, 1)
                        )
                    )
                    .append(" avg wait]\n");
                ...
                for (IoSite io : relatedIo) {
                    ...
                    sb.append(step++)
                        ...
                        .append(
                            SchemaUtil.formatDuration(
                                io.totalDurationMs / Math.max(io.count, 1)
                            )
                        )
                        .append(" avg]\n");
                }
        ...
        if (step == 1) {
            sb.append(
                "No clear bottleneck chain identified from available data.\n"
            );
        }
        sb.append("```\n\n");
    }

    private void appendCpuGcCorrelation(
        StringBuilder sb,
        IItemCollection events,
        int topN
    ) {
        ...
        if (avgCpu != null) sb.append(
            String.format(
                "| Avg CPU Load | %.1f%% |\n",
                avgCpu.doubleValue() * 100
            )
        );
        if (maxCpu != null) sb.append(
            String.format(
                "| Max CPU Load | %.1f%% |\n",
        ...
        sb.append("| Total GC Pause Time | ")
            .append(JfrAnalysisService.display(totalGcPause))
            .append(" |\n\n");
