...
public final class CompareRecordingsTool {
    ...
    private String analyze(String baselinePath, String targetPath)
        throws IOException {
        ...
        CompletableFuture<List<DeltaResult>> cpuDeltasTask =
        ...
        CompletableFuture<List<DeltaResult>> allocDeltasTask =
            CompletableFuture.supplyAsync(() ->
                calculateEventDeltas(
                    bCtx,
                    tCtx,
        ...
        CompletableFuture<List<DeltaResult>> contentionDeltasTask =
            CompletableFuture.supplyAsync(() ->
                calculateEventDeltas(
                    bCtx,
                    tCtx,
        ...
        CompletableFuture<List<DeltaResult>> exceptionDeltasTask =
            CompletableFuture.supplyAsync(() ->
                calculateEventDeltas(
                    bCtx,
                    tCtx,
    ...
    private Map<String, List<String>> calculateMetrics(
        ...
        for (List<MetricDef> group : groups.values()) {
            ...
            Map<MetricDef, IQuantity> bVals = computeMetricsBatch(
                bEvents,
                group
            );
            Map<MetricDef, IQuantity> tVals = computeMetricsBatch(
                tEvents,
                group
            );
    ...
    private Map<MetricDef, IQuantity> computeMetricsBatch(
        IItemCollection events,
        List<MetricDef> defs
    ) {
        ...
        boolean needPercentile = defs
            .stream()
            .anyMatch(d -> d.type == AggType.P95 || d.type == AggType.P99);
    ...
    private IQuantity getMetricValue(IItemCollection events, MetricDef def) {
        ...
        return switch (def.type) {
            ...
            case P95 -> JfrItemUtils.percentileQuantity(
                events,
                def.attrId,
                95.0
            );
            case P99 -> JfrItemUtils.percentileQuantity(
                events,
                def.attrId,
                99.0
            );
            case COUNT -> UnitLookup.NUMBER_UNITY.quantity(
                JfrItemUtils.count(events)
            );
        };
    }
    ...
    private AnalysisContext getContext(String path) throws IOException {
        ...
        double durationSec = (start != null && end != null)
            ? end.subtract(start).doubleValueIn(UnitLookup.SECOND)
            : 1.0;
    ...
    private List<DeltaResult> calculateEventDeltas(
    ...
    private void appendDeltaSection(
        ...
        List<DeltaResult> regressions = deltas
            .stream()
            .filter(d -> d.delta > 0)
            .limit(5)
            .toList();
        List<DeltaResult> improvements = deltas
            .stream()
            .filter(d -> d.delta < 0)
            .sorted(Comparator.comparingDouble(DeltaResult::delta))
            .limit(3)
            .toList();
    ...
    private String formatValue(double val, String unit) {
        ...
        if (unit.equals("Bytes/sec")) {
            ...
            return (
                prefix +
                SchemaUtil.formatDuration((long) (Math.abs(val) / 1_000_000)) +
                "/s"
            );
        }
