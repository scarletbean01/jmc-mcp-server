...
public final class ThreadCpuTool {
    ...
    public String analyze(String filePath, String startTimeStr, String endTimeStr, String packagePrefix, int topN) throws IOException {
        ...
        for (IItemIterable iterable : samples) {
            ...
            if (threadAccessor != null) {
                for (IItem item : iterable) {
                    ...
                    ThreadStats stats = threadStatsMap.computeIfAbsent(threadName, k -> new ThreadStats(k));
                    stats.samples++;
                    totalSamples++;
        ...
        var sortedThreads = threadStatsMap.values().stream()
        ...
        for (ThreadStats stats : sortedThreads) {
            ...
            String states = stats.stateCounts.entrySet().stream()
        ...
        for (ThreadStats stats : sortedThreads) {
            ...
            stats.methodCounts.entrySet().stream()
                    .sorted(Map.Entry.<String, Long>comparingByValue().reversed())
                    .limit(5)
                    .forEach(entry -> {
