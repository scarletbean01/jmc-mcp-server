        ...
        return SyncToolSpecification.builder()
            .tool(
                McpSchema.Tool.builder()
                    .name(NAME)
                    .description(
                        "Provide CPU flame graph data including thread states and hottest call paths."
                    )
                    .inputSchema(
                        SchemaUtil.objectSchema(
                            SchemaUtil.props(
                                ...
                                "top_n",
                                SchemaUtil.intProp(
                                    "Number of top call paths and methods (default 20)",
                                    20
                                ),
                                "async",
                                SchemaUtil.boolProp(
            ...
            for (IItem item : iterable) {
                ...
                if (stackAccessor != null) {
                    ...
                    if (stackObj instanceof IMCStackTrace trace) {
                        ...
                        if (
                            trace.getFrames() != null &&
                            !trace.getFrames().isEmpty()
                        ) {
                            IMCMethod method = trace
                                .getFrames()
                                .get(0)
                                .getMethod();
                            if (method != null) {
