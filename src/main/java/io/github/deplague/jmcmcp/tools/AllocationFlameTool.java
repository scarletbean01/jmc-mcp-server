    ...
    public String analyze(String filePath, String startTimeStr, String endTimeStr, String packagePrefix, int topN) throws Exception {
        ...
        for (String typeId : new String[]{"jdk.ObjectAllocationInNewTLAB", "jdk.ObjectAllocationOutsideTLAB"}) {
            ...
            for (IItemIterable iterable : allocs) {
                ...
                IMemberAccessor<IQuantity, IItem> sizeAccessor = JfrItemUtils.getAccessor(iterable.getType(), typeId.contains("Outside") ? "allocationSize" : "tlabSize");
        ...
        sb.append("- **Total Allocated Bytes:** ").append(SchemaUtil.formatBytes(totalBytes)).append("\n\n");
        ...
        pathDist.entrySet().stream()
                ...
                .limit(topN)
                .forEach(e -> sb.append(String.format("| %s | %.1f%% | `%s` |\n", SchemaUtil.formatBytes(e.getValue()), (e.getValue() * 100.0) / finalTotal, e.getKey().replace("\n", "`<br>`"))));
