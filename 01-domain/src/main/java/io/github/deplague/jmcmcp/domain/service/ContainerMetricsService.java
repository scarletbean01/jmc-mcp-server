...
public final class ContainerMetricsService {
    ...
    public ContainerMetricsResult analyze(IItemCollection events) {
        ...
        if (configEvents.hasItems()) {
            ...
            if (itemOpt.isPresent()) {
                ...
                config = of(new ContainerConfig(
                        jfrAccessorRepository.getMember(item, "cpuShares").map(Object::toString),
        ...
        if (cpuEvents.hasItems()) {
            var stats = jfrQuantityAggregator.batchStats(cpuEvents, "cpuTime");
        ...
        if (memEvents.hasItems()) {
            var memStats = jfrQuantityAggregator.batchStats(memEvents, "memoryUsage");
            var swapStats = jfrQuantityAggregator.batchStats(memEvents, "swapUsage");
