...
public final class DirectBuffersService {
    ...
    public DirectBufferResult analyze(IItemCollection events) {
        ...
        var countStats = jfrQuantityAggregator.batchStats(dbEvents, "directBufferCount");
        IQuantity minCount = countStats.get("min");
        IQuantity avgCount = countStats.get("avg");
        IQuantity maxCount = countStats.get("max");
        var memStats = jfrQuantityAggregator.batchStats(dbEvents, "directMemoryUsed");
        IQuantity maxUsed = memStats.get("max");
        IQuantity maxCapacity = jfrQuantityAggregator.maxQuantity(dbEvents, "directTotalCapacity");
        ...
        for (IItemIterable iterable : props) {
            ...
            var keyAcc = jfrAccessorRepository.getAccessor(type, "key");
            var valueAcc = jfrAccessorRepository.getAccessor(type, "value");
        ...
        for (IItemIterable iterable : dbEvents) {
            ...
            IType<?> type = iterable.getType();
            var countAcc = jfrAccessorRepository.getAccessor(type, "directBufferCount");
            var capAcc = jfrAccessorRepository.getAccessor(type, "directTotalCapacity");
            var usedAcc = jfrAccessorRepository.getAccessor(type, "directMemoryUsed");
