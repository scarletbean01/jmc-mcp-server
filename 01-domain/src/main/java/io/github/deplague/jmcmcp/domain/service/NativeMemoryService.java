...
public final class NativeMemoryService {
    ...
    public NativeMemoryResult analyze(IItemCollection events) {
        ...
        long libCount = jfrQuantityAggregator.count(nativeLibs);

        IItemCollection heapSummary = events.apply(type("jdk.GCHeapSummary"));
        IQuantity maxHeap = jfrQuantityAggregator.maxQuantity(heapSummary, "heapSize");
        ...
        for (IItemIterable iterable : props) {
            ...
            IMemberAccessor<String, IItem> keyAccessor = jfrAccessorRepository.getAccessor(type1, "key");
            IType<?> type = iterable.getType();
            IMemberAccessor<String, IItem> valueAccessor = jfrAccessorRepository.getAccessor(type, "value");
        ...
        if (libCount > 0) {
            ...
            for (IItemIterable iterable : nativeLibs) {
                IType<?> type = iterable.getType();
                IMemberAccessor<String, IItem> nameAccessor = jfrAccessorRepository.getAccessor(type, "name");
                if (nameAccessor != null) {
                    for (IItem item : iterable) {
                        if (count++ >= 50) break;
                        String name = nameAccessor.getMember(item);
                        String path = jfrAccessorRepository.getMember(item, "topLevelPath")
                                .map(Object::toString)
                                .orElse("N/A");
