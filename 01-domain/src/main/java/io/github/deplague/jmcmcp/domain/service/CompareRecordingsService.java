...
import static io.github.deplague.jmcmcp.domain.service.JfrValueConverter.toDouble;
...
public final class CompareRecordingsService {
    ...
    private Map<RecordingComparisonMetric, IQuantity> computeMetricsBatch(
        ...
        if (attrId == null) {
            long totalCount = jfrQuantityAggregator.count(events);
        ...
        for (IItemIterable iterable : events) {
            ...
            IMemberAccessor<Object, IItem> accessor = jfrAccessorRepository.getAccessor(type, attrId);
        ...
        for (String typeId : typeIds) {
            ...
            for (IItemIterable iterable : events) {
                ...
                if (keyAttr.equals(
                    ...
                    IType<?> type = iterable.getType();
                    keyAccessor = jfrAccessorRepository.getAccessor(type, "stackTrace");
                } else {
                    IType<?> type = iterable.getType();
                    keyAccessor = jfrAccessorRepository.getAccessor(type, keyAttr);
                }
                IMemberAccessor<Object, IItem> valAccessor;
                if (valueAttr != null) {
                    IType<?> type = iterable.getType();
                    valAccessor = jfrAccessorRepository.getAccessor(type, valueAttr);
                } else {
                    valAccessor = null;
                }
                ...
                if (typeId.contains("ObjectAllocation")) {
                    IType<?> type = iterable.getType();
                    valAccessor = jfrAccessorRepository.getAccessor(type, typeId.contains("Outside")
                            ? "allocationSize"
                            : "tlabSize");
                }
